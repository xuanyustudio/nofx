...
type ConfigFile struct {
	...
	ProxyURL           string            `json:"proxy_url"` // 代理地址，如: http://127.0.0.1:7890
...
func syncConfigToDatabase(database *config.Database, configFile *ConfigFile) error {
	...
	if configFile.ProxyURL != "" {
		configs["proxy_url"] = configFile.ProxyURL
	}
...
func main() {
	...
	// 设置代理（如果配置了）
	proxyURL, _ := database.GetSystemConfig("proxy_url")
	if proxyURL != "" {
		// 设置市场数据代理
		market.SetProxy(proxyURL)
		// 设置币安交易客户端代理（通过环境变量）
		os.Setenv("BINANCE_PROXY", proxyURL)
		log.Printf("✓ 已配置全局代理: %s", proxyURL)
	}
	...
	// 启动数据库中标记为运行状态的交易员
	log.Println()
	log.Println("🔍 检查数据库中标记为运行状态的交易员...")
	if err := traderManager.StartRunningTradersFromDatabase(database); err != nil {
		log.Printf("⚠️ 启动交易员失败: %v", err)
	}
	log.Println()
